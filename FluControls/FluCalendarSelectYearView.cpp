    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            ...
            connect(label, &FluCalendarItem::clicked, [=]() {
                ...
                // itemDate.month());
                // m_parentView->getSelectMonthView()->setYearMonth(itemDate.year(),
                // itemDate.month()); m_parentView->getViewTitle()->setYearMonth(itemDate.year(),
                // itemDate.month());
...
{
    FluStyleSheetUitls::setQssByFileName("FluCalendarSelectYearView.qss", this,
