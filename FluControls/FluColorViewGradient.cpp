...
bool FluColorViewGradient::findColor(QColor color, QPoint &point)
{
    ...
    for (int i = 0; i < 256; i++) {
        for (int j = 0; j < 256; j++) {
            ...
            // LOG_DEBUG << "cur color:" << curColor.red() << "," << curColor.green() << "," <<
            // curColor.blue();
            if (atRange(color.red(), curColor.red(), 2)
                && atRange(color.green(), curColor.green(), 2)
                && atRange(color.blue(), curColor.blue(), 2)) {
                point.setX(i);
                point.setY(j);
                bFind = true;
                break;
    ...
    // {
    //     auto color = colors[i];
    //     LOG_DEBUG << "find color:" << color.red() << "," << color.green() << ", " <<
    //     color.blue();
    // }
