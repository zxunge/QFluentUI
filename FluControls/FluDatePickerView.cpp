...
FluDatePickerView::FluDatePickerView(QWidget *parent /*= nullptr*/)
    : FluWidget(parent), m_bFirstShow(true), m_mask(nullptr)
{
    ...
    // set month data;
    std::vector<QString> datas{ tr("January"),   tr("February"), tr("March"),    tr("April"),
    ...
    connect(m_monthView, &FluLoopView::visibaleMidIndexChanged, [=](int nIndex) {
        int nDays = getMonthDays(m_yearView->getVisibleMidIndex() + 1924,
                                 m_monthView->getVisibleMidIndex() + 1);
    ...
    connect(m_dayView, &FluLoopView::visibaleMidIndexChanged,
            [=](int nIndex) { m_mask->setItemText(1, m_dayView->getCurrentText()); });
...
void FluDatePickerView::onThemeChanged()
{
    ...
    m_cancelBtn->setIcon(FluIconUtils::getFluentIconPixmap(FluAwesomeType::Cancel,
                                                           FluThemeUtils::getUtils()->getTheme()));
    FluStyleSheetUitls::setQssByFileName("FluDatePickerView.qss", this,
                                         FluThemeUtils::getUtils()->getTheme());
}
