...
FluVNavigationIconTextItem::FluVNavigationIconTextItem(QWidget *parent /*= nullptr*/)
    : FluVNavigationItem(parent)
{
    ...
    //  QString qss =
    //  FluStyleSheetUitls::getQssByFileName("../StyleSheet/light/FluNavigationIconTextItem.qss");
    //  setStyleSheet(qss);
...
FluVNavigationIconTextItem::FluVNavigationIconTextItem(QIcon icon, QString text,
                                                       QWidget *parent /*= nullptr*/)
    : FluVNavigationIconTextItem(parent)
{
...
FluVNavigationIconTextItem::FluVNavigationIconTextItem(FluAwesomeType awesomeType, QString text,
                                                       QWidget *parent /*= nullptr*/)
    : FluVNavigationIconTextItem(parent)
{
...
FluVNavigationIconTextItem::FluVNavigationIconTextItem(FluVNavigationIconTextItem *item,
                                                       QWidget *parent /*= nullptr*/)
    : FluVNavigationIconTextItem(parent)
{
    itemClone(item);
...
int FluVNavigationIconTextItem::calcItemW1Width()
{
    ...
    int nCurentWidth = margins.left() + nIndicatorWidth + nIconWidth + nSpacing + nLabelWidth
            + nArrowWidth + margins.right() + 36 * getDepth();
...
int FluVNavigationIconTextItem::calcItemWidth()
{
    ...
    for (int i = 0; i < nLayoutCount; i++) {
        if (getState() != FluVNavigationState::Expanded
            && getState() != FluVNavigationState::Expanding) {
            break;
        }
...
int FluVNavigationIconTextItem::calcItemW2Height(FluVNavigationIconTextItem *item)
{
    ...
    nH = nH + 5 * (nLayoutCount - 1) + m_vLayout1->contentsMargins().top()
            + m_vLayout1->contentsMargins().bottom();
    return nH;
}
...
void FluVNavigationIconTextItem::adjustItemHeight(FluVNavigationIconTextItem *item)
{
    ...
    item->setFixedHeight(item->m_wrapWidget1->height() + item->m_wrapWidget2->height()
                         + m_vMainLayout->spacing() + m_vMainLayout->contentsMargins().top()
                         + m_vMainLayout->contentsMargins().bottom());
...
void FluVNavigationIconTextItem::onItemClicked()
{
    ...
    if ((navView != nullptr && m_bDown && navView->isLong())
        || (navView == nullptr && isLong() && m_bDown)) {
        ...
        if (m_items.size() > 0) {
            ...
            nH = nH + m_vLayout1->spacing() * (nLayout1Count - 1)
                    + m_vLayout1->contentsMargins().top() + m_vLayout1->contentsMargins().bottom();
            m_wrapWidget2->setFixedHeight(nH);
            m_wrapWidget2->show();
            setFixedHeight(m_wrapWidget1->height() + m_wrapWidget2->height()
                           + m_vMainLayout->contentsMargins().top()
                           + m_vMainLayout->contentsMargins().bottom() + m_vMainLayout->spacing());
        }
