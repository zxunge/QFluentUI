...
public:
    ...
    void initContentWidget()
    {
        ...
        connect(colorViewGradient, &FluColorViewGradient::colorChanged,
                [=](QColor color) { colorViewHHandle->setColor(color); });
        ...
        connect(rEdit, &FluLineEdit::textChanged, this,
                [=](const QString &rValue) { onRgbValueChanged(); });

        connect(gEdit, &FluLineEdit::textChanged, this,
                [=](const QString &gValue) { onRgbValueChanged(); });

        connect(bEdit, &FluLineEdit::textChanged, this,
                [=](const QString &bValue) { onRgbValueChanged(); });
    ...
    QColor getColor() { return colorViewHHandle->getColor(); }
    ...
    {
        if (m_parentWidget != nullptr && watched == m_parentWidget
...
public slots:
    ...
    void onThemeChanged()
    {
        FluStyleSheetUitls::setQssByFileName("FluColorView.qss", this,
