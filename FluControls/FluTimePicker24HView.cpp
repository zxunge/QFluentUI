...
FluTimePicker24HView::FluTimePicker24HView(QWidget *parent /*= nullptr*/)
    : FluWidget(parent), m_bFirstShow(true)
{
    ...
    connect(m_hourView, &FluLoopView::visibaleMidIndexChanged,
            [=](int nIndex) { m_mask->setItemText(0, m_hourView->getCurrentText()); });

    connect(m_minuteView, &FluLoopView::visibaleMidIndexChanged,
            [=](int nIndex) { m_mask->setItemText(1, m_minuteView->getCurrentText()); });
...
void FluTimePicker24HView::onThemeChanged()
{
    if (FluThemeUtils::isLightTheme()) {
        ...
        m_cancelBtn->setIcon(
                FluIconUtils::getFluentIconPixmap(FluAwesomeType::Cancel, FluTheme::Light));

        // FluStyleSheetUitls::setQssByFileName("../StyleSheet/light/FluTimePicker24HView.qss",
        // this);
    } else {
        m_okBtn->setIcon(FluIconUtils::getFluentIcon(FluAwesomeType::Accept, FluTheme::Dark));
        m_cancelBtn->setIcon(
                FluIconUtils::getFluentIconPixmap(FluAwesomeType::Cancel, FluTheme::Dark));
        // FluStyleSheetUitls::setQssByFileName("../StyleSheet/dark/FluTimePicker24HView.qss",
        // this);
    }
    FluStyleSheetUitls::setQssByFileName("FluTimePicker24HView.qss", this,
