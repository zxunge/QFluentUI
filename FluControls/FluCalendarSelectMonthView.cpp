...
FluCalendarSelectMonthView::FluCalendarSelectMonthView(QWidget *parent /*= nullptr*/)
    : FluWidget(parent)
{
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            ...
            connect(getItem(i * 4 + j), &FluCalendarItem::clicked, [=]() {
                ...
                m_parentView->getSelectDayView()->getMonthView()->setYearMonth(itemDate.year(),
                                                                               itemDate.month());
...
void FluCalendarSelectMonthView::setYearMonth(int nYear, int nMonth)
{
    ...
    for (int i = 0; i < 16; i++) {
        ...
        if (getItem(i)->getCurDate().year() == QDate::currentDate().year()
            && getItem(i)->getCurDate().month() == QDate::currentDate().month()) {
...
{
    FluStyleSheetUitls::setQssByFileName("FluCalendarSelectMonthView.qss", this,
